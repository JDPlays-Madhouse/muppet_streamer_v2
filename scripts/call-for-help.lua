...
local CommandName = "muppet_streamer_v2_call_for_help"

CallForHelp.CreateGlobals = function()
    storage.callForHelp = storage.callForHelp or {} ---@class CallForHelp_Global
    storage.callForHelp.nextId = storage.callForHelp.nextId or 0 ---@type uint
    storage.callForHelp.pathingRequests = storage.callForHelp.pathingRequests or {} ---@type table<uint, CallForHelp_PathRequestObject> # Key'd to the pathing request Ids,
    storage.callForHelp.callForHelpIds = storage.callForHelp.callForHelpIds or {} ---@type table<uint, CallForHelp_CallForHelpObject> # Key'd to the callForHelp Ids.
end

CallForHelp.OnLoad = function()
    CommandsUtils.Register("muppet_streamer_v2_call_for_help", {"api-description.muppet_streamer_v2_call_for_help"},
        CallForHelp.CallForHelpCommand, true)
    EventScheduler.RegisterScheduledEventType("CallForHelp.CallForHelp", CallForHelp.CallForHelp)
    Events.RegisterHandlerEvent(defines.events.on_script_path_request_finished,
        "CallForHelp.OnScriptPathRequestFinished", CallForHelp.OnScriptPathRequestFinished)
...
CallForHelp.CallForHelpCommand = function(command)
    local commandData = CommandsUtils.GetSettingsTableFromCommandParameterString(command.parameter, true, CommandName,
        {"delay", "target", "arrivalRadius", "callRadius", "sameSurfaceOnly", "sameTeamOnly", "blacklistedPlayerNames",
         "whitelistedPlayerNames", "callSelection", "number", "activePercentage", "suppressMessages"})
    ...
    if not CommandsUtils.CheckNumberArgument(arrivalRadius, "double", false, CommandName, "arrivalRadius", 1, nil,
        command.parameter) then
        return
    ...
    if not CommandsUtils.CheckNumberArgument(callRadius, "double", false, CommandName, "callRadius", 1, nil,
        command.parameter) then
        return
    ...
    if not CommandsUtils.CheckStringArgument(blacklistedPlayerNames_string, false, CommandName,
        "blacklistedPlayerNames", nil, command.parameter) then
        return
    ...
    if not CommandsUtils.CheckStringArgument(whitelistedPlayerNames_string, false, CommandName,
        "whitelistedPlayerNames", nil, command.parameter) then
        return
    ...
    if not CommandsUtils.CheckStringArgument(callSelection_string, true, CommandName, "callSelection", CallSelection,
        command.parameter) then
        return
    ...
    if not CommandsUtils.CheckNumberArgument(number, "int", false, CommandName, "number", 0, MathUtils.uintMax,
        command.parameter) then
        return
    ...
    if not CommandsUtils.CheckNumberArgument(activePercentage, "double", false, CommandName, "activePercentage", 0, nil,
        command.parameter) then
        return
    ...
    if number == 0 and activePercentage == 0 then
        CommandsUtils.LogPrintError(CommandName, nil, "either number or activePercentage must be provided",
            command.parameter)
        return
    end
    ...
    storage.callForHelp.nextId = storage.callForHelp.nextId + 1
    ---@type CallForHelp_DelayedCommandDetails
    local delayedCommandDetails = {
        callForHelpId = storage.callForHelp.nextId,
        target = target,
        arrivalRadius = arrivalRadius,
        callRadius = callRadius,
        sameTeamOnly = sameTeamOnly,
        sameSurfaceOnly = sameSurfaceOnly,
        blacklistedPlayerNames = blacklistedPlayerNames,
        whitelistedPlayerNames = whitelistedPlayerNames,
    ...
    if scheduleTick ~= -1 then
        EventScheduler.ScheduleEventOnce(scheduleTick, "CallForHelp.CallForHelp", storage.callForHelp.nextId,
            delayedCommandDetails)
    else
        ---@type UtilityScheduledEvent_CallbackObject
        local eventData = {
            tick = command.tick,
            name = "CallForHelp.CallForHelp",
            instanceId = storage.callForHelp.nextId,
...
CallForHelp.CallForHelp = function(eventData)
    ...
    local targetPlayerEntity, targetPlayerInVehicle = CallForHelp.GetTargetPlayersEntity(targetPlayer,
        data.suppressMessages)
    if targetPlayerEntity == nil then
        return
    end

    local targetPlayerPosition, targetPlayerSurface = targetPlayer.physical_position, targetPlayer.physical_surface
    ...
    if maxPlayers <= 0 then
        if not data.suppressMessages then
            game.print({"message.muppet_streamer_v2_call_for_help_no_players_found", targetPlayer.name})
        end
        return
    end
            ...
            if (not data.sameTeamOnly) or helpPlayer.force == targetPlayerForce then
                ...
                if (not data.sameSurfaceOnly or helpPlayer_surface == targetPlayerSurface) and
                    ...
                    if helpPlayer_surface ~= targetPlayerSurface then
                        ...
                        distance = PositionUtils.GetDistance(targetPlayerPosition, helpPlayer.physical_position)
                    end

                    if data.callRadius == nil or distance <= data.callRadius then
                        helpPlayersInRange[#helpPlayersInRange + 1] = {
                            player = helpPlayer,
                            distance = distance
                        }
                    end
    ...
    elseif data.callSelection == CallSelection.nearest then
        table.sort(helpPlayersInRange, function(a, b)
    ...
    if not data.suppressMessages then
        game.print({"message.muppet_streamer_v2_call_for_help_start", targetPlayer.name})
    end
    storage.callForHelp.callForHelpIds[data.callForHelpId] = {
        callForHelpId = data.callForHelpId,
        pendingPathRequests = {}
    }
    local helpPlayer_moved, freeTrainCarriage, freeTrainCarriage_index, helpPlayer_vehicle, helpPlayer_vehicle_type,
        helpPlayer_canGoInTargetsVehicle
    for _, helpPlayer in pairs(helpPlayers) do
        ...
        if not helpPlayer_moved then
            CallForHelp.PlanTeleportHelpPlayer(helpPlayer, data.arrivalRadius, targetPlayer, targetPlayerPosition,
                targetPlayerSurface, targetPlayerEntity, data.callForHelpId, 1, data.sameSurfaceOnly, data.sameTeamOnly,
                data.suppressMessages)
        end
    end
end
...
CallForHelp.PlanTeleportHelpPlayer = function(helpPlayer, arrivalRadius, targetPlayer, targetPlayerPosition,
    targetPlayerSurface, targetPlayerEntity, callForHelpId, attempt, sameSurfaceOnly, sameTeamOnly, suppressMessages)
    ...
    elseif pathRequestId ~= nil then
        ...
        storage.callForHelp.pathingRequests[pathRequestId] = pathRequestObject
        storage.callForHelp.callForHelpIds[callForHelpId].pendingPathRequests[pathRequestId] = pathRequestObject

        return
    elseif teleportResponse.errorNoValidPositionFound then
        -- No valid position was found to try and teleport too.
        if not suppressMessages then
            game.print({"message.muppet_streamer_v2_call_for_help_no_teleport_location_found", helpPlayer.name,
                        targetPlayer.name})
        end
        return
    elseif teleportResponse.errorTeleportFailed then
        ...
        CommandsUtils.LogPrintWarning(CommandName, nil,
            "Teleport action for player " .. helpPlayer.name .. " to position " ..
...
CallForHelp.OnScriptPathRequestFinished = function(event)
    ...
    -- Update the globals.
    storage.callForHelp.callForHelpIds[pathRequest.callForHelpId].pendingPathRequests[pathRequest.pathRequestId] = nil
    storage.callForHelp.pathingRequests[event.id] = nil
    ...
    local targetPlayerEntity =
        CallForHelp.GetTargetPlayersEntity(pathRequest.targetPlayer, pathRequest.suppressMessages)
    if targetPlayerEntity == nil then
        return
    end
    ...
    if event.path == nil then
        ...
        if pathRequest.attempt > MaxPathfinderAttemptsForTargetLocation then
            if not pathRequest.suppressMessages then
                game.print({"message.muppet_streamer_v2_call_for_help_no_teleport_location_found", helpPlayer.name,
                            pathRequest.targetPlayer.name})
            end
        else
            ...
            CallForHelp.PlanTeleportHelpPlayer(helpPlayer, pathRequest.arrivalRadius, pathRequest.targetPlayer,
                pathRequest.targetPlayerPosition, pathRequest.surface, targetPlayerEntity, pathRequest.callForHelpId,
                pathRequest.attempt, pathRequest.sameSurfaceOnly, pathRequest.sameTeamOnly, pathRequest.suppressMessages)
        end
    else
        ...
        if helpPlayer.force ~= pathRequest.helpPlayerForce then
            if not pathRequest.sameTeamOnly or
                (pathRequest.sameTeamOnly and helpPlayer.force == pathRequest.targetPlayer.force) then
                -- Only try again if the force setting still allows it.
                CallForHelp.PlanTeleportHelpPlayer(helpPlayer, pathRequest.arrivalRadius, pathRequest.targetPlayer,
                    pathRequest.targetPlayerPosition, pathRequest.surface, targetPlayerEntity,
                    pathRequest.callForHelpId, pathRequest.attempt, pathRequest.sameSurfaceOnly,
                    pathRequest.sameTeamOnly, pathRequest.suppressMessages)
            end
            CallForHelp.CheckIfCallForHelpCompleted(pathRequest)
            return
        end
        ...
        local currentPlayerPlacementEntity, currentPlayerPlacementEntity_isVehicle =
            PlayerTeleport.GetPlayerTeleportPlacementEntity(helpPlayer, nil)
        if currentPlayerPlacementEntity == nil then
            ...
            if not pathRequest.suppressMessages then
                game.print({"message.muppet_streamer_v2_call_for_help_teleport_action_failed", helpPlayer.name,
                            LoggingUtils.PositionToString(pathRequest.position)})
            end
            CallForHelp.CheckIfCallForHelpCompleted(pathRequest)
            return
        end
        ...
        if not pathRequest.surface.can_place_entity {
            name = currentPlayerPlacementEntity.name,
            position = pathRequest.position,
            direction = currentPlayerPlacementEntity_vehicleDirectionFacing,
            force = pathRequest.helpPlayerForce,
        ...
        local teleportSucceeded = PlayerTeleport.TeleportToSpecificPosition(helpPlayer, pathRequest.surface,
        ...
        if not teleportSucceeded then
            CommandsUtils.LogPrintWarning(CommandName, nil,
                "Teleport action for player " .. helpPlayer.name .. " to position " ..
                    LoggingUtils.PositionToString(pathRequest.position) .. " failed (Factorio request rejected)", nil)
            CallForHelp.CheckIfCallForHelpCompleted(pathRequest)
            return
        end
    end
...
CallForHelp.CheckIfCallForHelpCompleted = function(pathRequest)
    if next(storage.callForHelp.callForHelpIds[pathRequest.callForHelpId].pendingPathRequests) == nil then
        if not pathRequest.suppressMessages then
            game.print({"message.muppet_streamer_v2_call_for_help_stop", pathRequest.targetPlayer.name})
        end
        storage.callForHelp.callForHelpIds[pathRequest.callForHelpId] = nil
    end
end
...
CallForHelp.GetTargetPlayersEntity = function(targetPlayer, suppressMessages)
    ...
    if targetPlayerEntity == nil then
        ...
        if not suppressMessages then
            game.print({"message.muppet_streamer_v2_call_for_help_no_character", targetPlayer.name})
        end
        return nil, false
    end
